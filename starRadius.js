...
function fmtTeff(x) { return Math.round(x).toString(); }
...
function printLumBlock(luminosity, teff) {
  ...
  if (teff !== null && teff !== undefined) {
    ...
    console.log(`Luminosity (visual, L_Sun): ${fmt(Lv)}${inRange ? "" : "  (est. — Teff outside BC_V valid range)"}`);
  }
}
...
const MS_TABLE = {
  ...
  "K5":  [ 4440,-0.76,  0.701, 0.70],
  "K6":  [ 4300,-0.86,  0.669, 0.69],
  "K7":  [ 4100,-1.00,  0.630, 0.64],
  "K8":  [ 3990,-1.06,  0.615, 0.62],
  "K9":  [ 3930,-1.10,  0.608, 0.59],
  // M-type
  "M0":  [ 3850,-1.16,  0.588, 0.57],
  "M0.5":[ 3770,-1.27,  0.544, 0.54],
  "M1":  [ 3660,-1.39,  0.501, 0.50],
  "M1.5":[ 3620,-1.44,  0.482, 0.47],
  "M2":  [ 3560,-1.54,  0.446, 0.44],
  "M2.5":[ 3470,-1.64,  0.421, 0.40],
  "M3":  [ 3430,-1.79,  0.361, 0.37],
  "M3.5":[ 3270,-2.03,  0.300, 0.27],
  "M4":  [ 3210,-2.14,  0.274, 0.23],
  "M4.5":[ 3110,-2.40,  0.217, 0.162],
  "M5":  [ 2950,-2.70,  0.189, 0.132],
  "M5.5":[ 2800,-3.00,  0.168, 0.113],
  "M6":  [ 2700,-3.14,  0.148, 0.102],
  "M6.5":[ 2600,-3.28,  0.127, 0.090],
  "M7":  [ 2500,-3.49,  0.112, 0.083],
  "M7.5":[ 2400,-3.67,  0.107, 0.078],
  "M8":  [ 2300,-3.87,  0.101, 0.073],
  "M8.5":[ 2200,-4.07,  0.097, 0.070],
  "M9":  [ 2000,-4.60,  0.088, 0.079],
};
...
const MS_SEQUENCE = (() => {
  ...
  for (const key of Object.keys(MS_TABLE)) {
    ...
    out.push({ key, letter, subtype: numPart, Teff, logL, R, M,
               L: Math.pow(10, logL) });
  }
...
const MS_CLASSES = {
  ...
  G: { label: "G-type  (yellow star, Sun-like)",       Mmin: 0.80, Mmax: 1.04},
  K: { label: "K-type  (orange star, ε Eri-like)",     Mmin: 0.45, Mmax: 0.80},
...
function msLookup(classKey, subtypeVal) {
  ...
  const directKey = classKey + (Number.isInteger(subtypeVal) ? String(subtypeVal) :
  ...
  const classEntries = MS_SEQUENCE.filter(e => e.letter === classKey)
  ...
  for (let i = 0; i < classEntries.length - 1; i++) {
    if (classEntries[i].subtype <= subtypeVal &&
...
function massToSubtype(massSun) {
  ...
  return { classKey: best.letter, subtypeVal: best.subtype,
           subtypeStr: Number.isInteger(best.subtype) ? String(best.subtype) :
...
function ageAdjust(massSun, L_ZAMS, R_ZAMS, ageGyr) {
  ...
  return { L: L_ZAMS * (1 + 0.40 * f), R: R_ZAMS * (1 + 0.10 * f), t_MS };
}

function printMS(classKey, subtypeStr, subtypeVal, massSun, ageGyr) {
  const zams = msLookup(classKey, subtypeVal);
  if (!zams) { console.log("Could not retrieve ZAMS values."); return; }

  const hasAge = ageGyr !== null;
  let L, R, t_MS;
  if (hasAge) {
    const adj = ageAdjust(massSun, zams.L, zams.R, ageGyr);
    L = adj.L; R = adj.R; t_MS = adj.t_MS;
  } else {
    L = zams.L; R = zams.R; t_MS = mainSequenceLifetime(massSun, zams.L);
  }
  ...
  if (hasAge) {
    console.log(`Age:                   ${fmt(ageGyr)} Gyr  (${fmtPct(ageGyr/t_MS*100)}% through MS)`);
    console.log(`\n  -- ZAMS (t = 0) --`);
    console.log(`  Radius:              ${fmt(zams.R)} R_Sun  /  ${fmt(zams.R * R_SUN_KM)} km`);
...
function pmsLuminosity(massSun, R_ZAMS, L_ZAMS, ageMyr) {
  ...
  if (ageMyr < t_henyey) {
    ...
    const Teff = Teff_hay;
    return 4 * Math.PI * Math.pow(R * R_SUN_M, 2) * SIGMA * Math.pow(Teff, 4) / L_SUN;
  }
  ...
  return 4 * Math.PI * Math.pow(R * R_SUN_M, 2) * SIGMA * Math.pow(Teff_interp, 4) / L_SUN;
}
...
function printPMS(massSun, ageMyr) {
  ...
  const R_ZAMS = zams.R, L_ZAMS = zams.L;
  const t_KH    = pmsKelvinHelmholtzTimeMyr(massSun, R_ZAMS, L_ZAMS);
  ...
  console.log(`      approximation from Baraffe et al. (2015) and Palla & Stahler (1993).`);
  console.log(`      Uncertainties are ~15–30% due to accretion history and initial radius.`);
}
...
const POST_MS_PHASES = {
  "SG": {
    ...
    R_range:     [1.5, 5],
    L_range:     [3, 50],
    description: "H-shell burning begins; envelope expands, Teff drops. " +
  ...
  "RGB": {
    ...
    R_range:     [5, 200],
    L_range:     [10, 2000],
    description: "Degenerate He core; H-shell burning drives envelope expansion. " +
  ...
  "HB": {
    ...
    R_range:     [1, 15],
    L_range:     [40, 80],
    description: "He core burning. L ≈ 40–80 L_Sun nearly constant. " +
  ...
  "AGB": {
    ...
    R_range:     [50, 500],
    L_range:     [1000, 50000],
    description: "He+H double-shell burning; intense pulsations, strong mass loss. " +
  ...
  "RSG": {
    ...
    description: "Post-MS evolution of M > 8 M_Sun. Levesque et al. (2005) empirical " +
  ...
  "BSG": {
    ...
    description: "Post-MS / blue loop phase of massive stars. " +
...
function postMsEstimate(phase, massSun, FeH) {
  ...
  switch (phase) {
    case "SG":
      ...
      Teff = p.Teff_range[1] - f * (p.Teff_range[1] - p.Teff_range[0]) + 200 * feh;
      break;
    case "RGB":
      R    = p.R_range[0] * Math.pow(p.R_range[1]/p.R_range[0], 1 - f * 0.3);
      L    = p.L_range[0] * Math.pow(p.L_range[1]/p.L_range[0], 1 - f * 0.3)
    ...
    case "AGB":
      ...
      L    = p.L_range[0] * Math.pow(p.L_range[1]/p.L_range[0], f)
             * Math.pow(10, -0.2 * feh);
      Teff = p.Teff_range[1] - f * (p.Teff_range[1] - p.Teff_range[0]) + 150 * (-feh);
      break;
    case "RSG":
...
const WR_SUBTYPES = {
  "WN2-4":  { Teff_range:[100000,150000], R_range:[0.7,3.0],  L_range:[2e4,2e5],  M_range:[10,20],  desc:"Early WN; H-free, compact" },
  "WN5-6":  { Teff_range:[ 60000,100000], R_range:[1.5,6.0],  L_range:[1e5,5e5],  M_range:[15,30],  desc:"Mid WN; dominant Galactic WR class" },
  "WN7-8":  { Teff_range:[ 35000, 60000], R_range:[3.0,15.0], L_range:[2e5,1e6],  M_range:[15,40],  desc:"Late WN; often H-rich (WNL)" },
  "WN9-11": { Teff_range:[ 25000, 35000], R_range:[10.0,30.0],L_range:[3e5,1e6],  M_range:[20,80],  desc:"Very late WN (Ofpe/WN9); near Eddington" },
  "WC4-6":  { Teff_range:[ 60000,120000], R_range:[0.8,4.0],  L_range:[1e5,4e5],  M_range:[10,20],  desc:"Early WC; H+N depleted, C/O-rich" },
  "WC7-9":  { Teff_range:[ 35000, 60000], R_range:[3.0,10.0], L_range:[1e5,3e5],  M_range:[10,20],  desc:"Late WC; dust formation in WC9" },
  "WO1-2":  { Teff_range:[120000,200000], R_range:[0.5,2.0],  L_range:[2e5,5e5],  M_range:[10,20],  desc:"Oxygen sequence; pre-SN Ib/c progenitor" },
};
...
function printWR(subtypeKey, massSun) {
  ...
  if (!wt) { console.log("Unknown WR subtype."); return; }
  const f = Math.max(0, Math.min(1,
  ...
  console.log(`Mass:                  ${fmt(massSun)} M_Sun  [typical: ${wt.M_range[0]}–${wt.M_range[1]} M_Sun]`);
  console.log(`Radius:                ${fmt(R)} R_Sun  [${wt.R_range[0]}–${wt.R_range[1]} R_Sun]`);
  console.log(`                       ${fmt(R * R_SUN_KM)} km`);
  printLumBlock(L, Teff);
  console.log(`  (Typical L range:    ${wt.L_range[0].toExponential(1)}–${wt.L_range[1].toExponential(1)} L_Sun)`);
  console.log(`T_eff (stellar core):  ${fmtTeff(Teff)} K  [${wt.Teff_range[0]}–${wt.Teff_range[1]} K]`);
  console.log(`T_eff (S-B check):     ${fmtTeff(calcTeff(L, R))} K`);
  console.log(`Mean density:          ${fmt(calcDensity(massSun, R))} g/cm³`);
  console.log(`\nCalibration:  Crowther (2007) ARA&A 45, 177; Hamann et al. (2019).`);
  console.log(`Note: WR "radius" is the hydrostatic core radius (τ_Ross = 20).`);
...
function printWD(massSun, ageGyr) {
  ...
  if (L !== null) {
    ...
    printLumBlock(L, Teff);
    console.log(`T_eff (estimated):     ${fmtTeff(Teff)} K`);
    if      (Teff > 75000) console.log(`WD type hint:          DO/DAO  (very hot, He/H atmosphere)`);
    else if (Teff > 45000) console.log(`WD type hint:          DO      (hot He-dominated)`);
    else if (Teff > 12000) console.log(`WD type hint:          DA or DB (hydrogen or helium atmosphere)`);
    else if (Teff > 5000)  console.log(`WD type hint:          DC / DQ  (featureless or carbon bands)`);
    else                   console.log(`WD type hint:          Cold DC / DZ (very cool, >10 Gyr old)`);
  } else {
  ...
  console.log(`Note: composition (DA/DB/DC) and hydrogen layer mass affect radius by 1–15%.`);
  if (massSun > 1.30)
    console.log(`!  Mass close to Chandrasekhar limit — radius very small; extreme caution.`);
}
...
function printNS(massSun) {
  ...
  console.log(`              Riley et al. (2021); Choudhury et al. (2024) PSR J0437-4715.`);
  console.log(`              EOS: stiff nuclear χEFT model; R uncertainty ±1 km.`);
  console.log(`Note: Neutron stars emit as X-ray pulsars, magnetars, or radio pulsars.`);
  console.log(`      Luminosity depends on spin-down power / accretion — not calculated here.`);
}
...
function kerrRadii(massSun, aStar) {
  ...
  const T_H = (HBAR * C * C * C)
              / (8 * Math.PI * G * massSun * M_SUN_KG * K_B)
...
function printBH(massSun, aStar) {
  ...
  const spinLabel = isSchw ? "0 (Schwarzschild, non-rotating)"
  ...
  if (!isSchw) {
    console.log(`  (Schwarzschild R_S = 2 R_g = ${fmt(2 * kr.Rg)} km for comparison)`);
  }
  console.log(`\n  -- Other Key Radii --`);
  console.log(`Ergosphere (equatorial):${fmt(kr.r_ergo)} km  (= 2 R_g, spin-independent at equator)`);
  ...
  console.log(`                       (Purely theoretical; stellar BHs are far too cold to detect)`);
  console.log(`Accretion efficiency:  ${fmtPct(kr.eta * 100)}%`);
  console.log(`                       (Max energy extractable from accreting matter at ISCO)`);
  console.log(`\nNote: Radii assume equatorial plane, prograde orbits, and Kerr metric (uncharged BH).`);
  console.log(`      BHs have no surface — these are spacetime boundaries, not physical surfaces.`);
  if (spin > 0.998)
...
function bdRadius(massJup, ageGyr) {
  ...
  const r = 0.1008 * Math.pow(massJup / 40, -0.055) * Math.pow(t, -0.02290);
  return Math.max(0.080, Math.min(0.130, r));
}
function bdRadiusToMass(radiusSun, ageGyr) {
  const t = Math.max(0.001, ageGyr);
  const rAdj = radiusSun / (0.1008 * Math.pow(t, -0.02290));
  return 40 * Math.exp(-Math.log(Math.max(0.080/0.1008, Math.min(0.130/0.1008, rAdj))) / 0.055);
}
function bdLuminosity(massJup, ageGyr) {
  return 3.959e-5 * Math.pow(massJup/40, 3.082) * Math.pow(Math.max(0.001, ageGyr), -0.7659);
}
function bdTeff(massJup, ageGyr) {
  return 843.35 * Math.pow(massJup/40, 1.3601) * Math.pow(Math.max(0.001, ageGyr), -0.2183);
}
function bdTeffToMass(teff, ageGyr) {
  return 40 * Math.pow(teff / (843.35 * Math.pow(Math.max(0.001,ageGyr), -0.2183)), 1/1.3601);
}
...
function printBD(massJup, ageGyr) {
  ...
  console.log(`Mass:                  ${fmt(massJup)} M_Jupiter  /  ${fmt(massJup * M_JUP_IN_MSUN)} M_Sun`);
  console.log(`Age:                   ${fmt(ageGyr)} Gyr`);
  console.log(`Radius:                ${fmt(R)} R_Sun  /  ${fmt(R * R_SUN_KM)} km`);
  printLumBlock(L, Teff);
  console.log(`T_eff (spectroscopic): ${fmtTeff(Teff_tr)} K  (Baraffe cooling track — used for classification)`);
  console.log(`T_eff (blackbody):     ${fmtTeff(Teff_SB)} K  (Stefan-Boltzmann; higher due to non-blackbody BD atmospheres)`);
  console.log(`Mean density:          ${fmt(calcDensity(massJup * M_JUP_IN_MSUN, R))} g/cm³`);
  if (ageGyr < 0.1)  console.log(`\nNote: Age < 0.1 Gyr — still contracting. Luminosity may be underestimated.`);
  if (massJup < 5)   console.log(`\nNote: Mass < 5 M_Jup — likely a super-Jupiter, not a true brown dwarf.`);
  if (massJup > 80)  console.log(`\nNote: Mass > 80 M_Jup — approaches hydrogen-burning limit. Consider M-type stellar model.`);
  if (Teff_tr > 2200)console.log(`\nNote: T_eff > 2200 K — near M/L boundary; late M-type stellar model may be more appropriate.`);
}
...
function askRepeat() {
  ask("\nCalculate another object? (y/n): ", (a) => {
    ...
    else { console.log("Goodbye!"); rl.close(); }
  });
}
...
function askFloat(prompt, min, max, callback) {
  ask(prompt, (input) => {
    ...
    if (isNaN(v) || (min !== null && v < min) || (max !== null && v > max)) {
      console.log(`Invalid input — expected a number${min!==null?` ≥ ${min}`:""}${max!==null?` ≤ ${max}`:""}.`);
...
function askAgeGyr(t_MS, callback) {
  ...
  ask(`Enter age in Gyr${suffix} [Enter to skip]: `, (input) => {
    ...
    const v = parseFloat(input.trim());
    if (isNaN(v) || v < 0) { console.log("Invalid age."); return callback(null); }
    if (t_MS && v >= t_MS) {
      console.log(`!  Age ${fmt(v)} Gyr ≥ MS lifetime ${fmtLifetime(t_MS)} — using ZAMS values.`);
...
function askBDAge(callback) {
  askFloat("Enter age in Gyr (required for brown dwarf luminosity): ", 0.001, null, callback);
}
...
function askSubtype(classLabel, required, callback) {
  ...
  ask(`Enter spectral subtype for ${classLabel}-type (0–9, ≤1 dp, e.g. 2 or 5.9)${suffix}: `,
    (input) => {
...
function mainMenu() {
  console.log(`
╔════════════════════════════════════════════╗
║  Stellar Parameter Calculator  v2.0        ║
...
╚════════════════════════════════════════════╝`);
  ask("\nEnter phase number: ", (input) => {
    const v = input.trim();
    switch (v) {
      case "1": return pmsMenu();
      case "2": return msMenu();
      case "3": return postMsMenu();
      case "4": return wrMenu();
      case "5": return wdMenu();
      case "6": return nsMenu();
      case "7": return bhMenu();
      case "8": return bdMenu();
      default: console.log("Invalid."); return mainMenu();
    }
  });
}
...
function msMenu() {
  ...
  ask("Enter input mode: ", (m) => {
    switch (m.trim()) {
      case "1": return msFromSpectralType();
      case "2": return msFromMass();
      case "3": return msFromRadius();
      default: console.log("Invalid."); return msMenu();
    }
  });
}
...
function msFromMass() {
  ...
  ask("Enter class number: ", (input) => {
    const idx = parseInt(input.trim()) - 1;
    if (isNaN(idx) || idx < 0 || idx >= MS_CLASS_KEYS.length) {
      console.log("Invalid."); return msFromMass();
    }
    const classKey = MS_CLASS_KEYS[idx];
    ...
    askSubtype(classKey, false, (sub) => {
      ...
      ask(prompt, (massInput) => {
        ...
        if (massInput.trim() === "" && impliedMass) {
          ...
          if (isNaN(mass) || mass <= 0) { console.log("Invalid."); return msFromMass(); }
          if (impliedMass) {
            const dev = Math.abs(mass - impliedMass) / impliedMass;
            if (dev > 0.1)
              console.log(`Note: ${fmt(mass)} M_Sun deviates ${fmtPct(dev*100)}% from ${classKey}${sub.str}-implied ${fmt(impliedMass)} M_Sun.`);
          }
        }
        // Auto subtype if not provided
        const autoSub   = sub || (() => { const s = massToSubtype(mass); return { str: s.subtypeStr, val: s.subtypeVal }; })();
        const autoClass = sub ? classKey : massToClass(mass);
        const zams      = msLookup(autoClass, autoSub.val);
        const t_MS      = mainSequenceLifetime(mass, zams ? zams.L : 1);
        if (!sub) console.log(`  Calculated subtype: ${autoClass}${autoSub.str}  (from mass)`);
        askAgeGyr(t_MS, (age) => {
...
function msFromRadius() {
  ...
  MS_CLASS_KEYS.forEach((k,i) => console.log(`  ${i+1}) ${MS_CLASSES[k].label}`));
  ask("Enter class: ", (input) => {
    const idx = parseInt(input.trim()) - 1;
    if (isNaN(idx) || idx < 0 || idx >= MS_CLASS_KEYS.length) { console.log("Invalid."); return msFromRadius(); }
    const classKey = MS_CLASS_KEYS[idx];
    askSubtype(classKey, false, (sub) => {
      ask("\nRadius unit: 1) R_Sun  2) km\n: ", (u) => {
        ...
        askFloat(`Enter radius in ${label}: `, 0, null, (rIn) => {
          ...
          let best = MS_SEQUENCE[0], bestD = Math.abs(MS_SEQUENCE[0].R - rSun);
          for (const e of MS_SEQUENCE) {
            const d = Math.abs(e.R - rSun);
            if (d < bestD) { bestD = d; best = e; }
          }
          const mass = best.M;
          console.log(`  Nearest MS match: ${best.letter}${Number.isInteger(best.subtype)?best.subtype:best.subtype.toFixed(1)}V  M = ${fmt(mass)} M_Sun`);
          if (sub) {
            const implied = (msLookup(classKey, sub.val) || {}).M;
            if (implied) {
              const dev = Math.abs(mass - implied) / implied;
              if (dev > 0.1)
                console.log(`Note: derived mass ${fmt(mass)} deviates ${fmtPct(dev*100)}% from ${classKey}${sub.str}-implied ${fmt(implied)} M_Sun.`);
            }
          }
          ...
          askAgeGyr(t_MS, (age) => {
            printMS(best.letter, Number.isInteger(best.subtype)?String(best.subtype):best.subtype.toFixed(1), best.subtype, mass, age);
            askRepeat();
          });
        });
      });
...
function postMsMenu() {
  ...
  POST_MS_KEYS.forEach((k,i) => {
    ...
    console.log(`  ${i+1}) ${p.label.padEnd(35)} [M: ${p.massMin}–${p.massMax} M_Sun]`);
  });
  ask("Enter phase: ", (input) => {
    const idx = parseInt(input.trim()) - 1;
    if (isNaN(idx) || idx < 0 || idx >= POST_MS_KEYS.length) { console.log("Invalid."); return postMsMenu(); }
    const phase = POST_MS_KEYS[idx];
    const p = POST_MS_PHASES[phase];
    askFloat(`Enter mass (M_Sun, typical ${p.massMin}–${p.massMax}): `, 0.3, 60, (mass) => {
      ask(`Enter metallicity [Fe/H] (e.g. 0.0 = solar, -1.0 = metal-poor, +0.3 = metal-rich) [Enter for solar]: `,
        (fehInput) => {
...
function wrMenu() {
  ...
  WR_KEYS.forEach((k,i) => {
    ...
    console.log(`         M: ${w.M_range[0]}–${w.M_range[1]} M_Sun  Teff: ${(w.Teff_range[0]/1000).toFixed(0)}–${(w.Teff_range[1]/1000).toFixed(0)} kK`);
  });
  ask("Enter subtype number: ", (input) => {
    const idx = parseInt(input.trim()) - 1;
    if (isNaN(idx) || idx < 0 || idx >= WR_KEYS.length) { console.log("Invalid."); return wrMenu(); }
    const key = WR_KEYS[idx];
    const w   = WR_SUBTYPES[key];
    askFloat(`Enter mass (M_Sun, typical ${w.M_range[0]}–${w.M_range[1]}): `, 1, 100, (mass) => {
      printWR(key, mass);
      askRepeat();
    });
  });
...
function bhMenu() {
  ...
  askFloat("Enter BH mass (M_Sun, e.g. 5–100): ", 0.1, null, (mass) => {
    ask("Enter spin parameter a* (0 = non-rotating, 1 = maximal spin) [Enter for 0]: ", (spinInput) => {
...
function bdMenu() {
  ...
  ask("Input mode: ", (m) => {
    switch (m.trim()) {
      case "1": return bdFromMass();
      case "2": return bdFromSpectralType();
      case "3": return bdFromRadius();
      default:  console.log("Invalid."); return bdMenu();
    }
  });
}

function bdFromMass() {
  askFloat("Enter mass (M_Jupiter, L/T: ~13–80, Y: ~5–25): ", 0.1, 200, (mass) => {
    askBDAge((age) => { printBD(mass, age); askRepeat(); });
  });
}
...
function bdFromSpectralType() {
  ...
  ask("Enter class: ", (input) => {
    ...
    if (!bdKey) { console.log("Invalid."); return bdFromSpectralType(); }
    ...
    askSubtype(bdKey, true, (sub) => {
      ...
      askBDAge((age) => {
        ...
        if (mass > 80) console.log(`  Note: > 80 M_Jup — near hydrogen-burning limit.`);
        printBD(mass, age);
        askRepeat();
      });
    });
